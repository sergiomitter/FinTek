          ...
          <form onSubmit={handleUpdatePassword} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                <input type={showNewPassword ? "text" : "password"} required autoFocus className="w-full h-12 bg-background-dark border border-surface-highlight rounded-xl pl-4 pr-12 text-white focus:ring-2 focus:ring-primary font-bold" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
                <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary"><span className="material-symbols-outlined">{showNewPassword ? 'visibility_off' : 'visibility'}</span></button>
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Confirmar Senha</label>
              <div className="relative">
                <input type={showConfirmPassword ? "text" : "password"} required className="w-full h-12 bg-background-dark border border-surface-highlight rounded-xl pl-4 pr-12 text-white focus:ring-2 focus:ring-primary font-bold" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-text-secondary"><span className="material-symbols-outlined">{showConfirmPassword ? 'visibility_off' : 'visibility'}</span></button>
              </div>
            </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-background-dark p-6">
        <div className="w-full max-w-md bg-surface-dark border border-surface-highlight rounded-3xl p-10 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="text-center mb-8">
            ...
            <p className="text-text-secondary text-sm mt-2 font-medium">Insira seu e-mail.</p>
          </div>
    ...
    <div className="min-h-screen flex items-center justify-center bg-background-dark p-6">
      <div className="w-full max-w-md bg-surface-dark border-2 border-primary/50 rounded-3xl p-10 shadow-2xl animate-in fade-in duration-500">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-secondary"><span className="material-symbols-outlined">{showPassword ? 'visibility_off' : 'visibility'}</span></button>
            </div>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-surface-highlight/50 text-center relative">
          ...
          <div className="mt-4"><p className="text-primary text-[10px] font-black uppercase tracking-[0.2em] animate-pulse">v1.2.8 - VERIFIED</p></div>
        </div>
      </div>
    </div>
  );
